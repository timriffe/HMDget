...
selectyears <- function(A){
	...
	for (i in 1:length(Variables)){
		...
		years.in.all.Countries <- sort(names(CT[which(CT==length(Countries))]))
		...
		if(length(years.in.all.Countries)==length(years.in.any.Country)){
			...
			if (ANYALL=="in ANY selected country"){
				...
				rangemati[which(is.infinite(rangemati)==TRUE)] <- NA
				print(rangemati)
				wantedyears <- select.list(years.in.any.Country,preselect=NULL,multiple=TRUE,title=paste("Select Years,",Variables[i]))
			} else {
...
droptadj <- function(drop.tadj,item,wanteditem){
	if (drop.tadj==TRUE & wanteditem == "Population"){
		isel<-grep("-",item$Year);
		if (length(isel)>0){
			item <- item[-isel,]  
		}
...
formatoutput <- function(inlist,format,column){
	...
	if (format==3){
		...
		for (i in 1:length(wanteditems)){
			...
			colindices <- which(allcolumnsitemi%in%columnsitemi)
			...
			for (j in 1:length(columnsitemi)){
				...
				cntryvec <- c(); agesvec <- c()
				for (k in 1:length(inlist)){
					varicoljctryk <- matrix(inlist[[k]][[i]][,colindices[j]],ncol=Nyears)
				...
				if (Nages==1) {varicolumnj <- varicolumnj[,-which(colnames(varicolumnj)=="Age")]}
	...
	if (format==4){
		#A <- formatoutput(output,format=format,column=column)
		#inlist <- output
		#column <- "Female"
		#wanteditems <- "Deaths_lexis"
		if (length(inlist)==1 & length(inlist[[1]])==1){
			possiblecolumns <- colnames(inlist[[1]][[1]])[colnames(inlist[[1]][[1]]) %in% c("Year","Age") ==FALSE]
			if (is.null(column) | (is.null(column)==FALSE & any(column %in% possiblecolumns)==FALSE)){
			...
			colnames(output) <- Years ; rownames(output) <- Ages
	...
	if (is.data.frame(output)==FALSE & length(inlist)==1 & length(inlist[[1]])==1 & format!=3 & format!=4){
...
HMD2Rlocal <- function(countries=NULL,wanteditems=NULL, drop.tadj=TRUE, years=TRUE, column=NULL, format=0,path=NULL,syntax=TRUE){
	...
	for (i in 1:length(Countries)){
		ctry.files <- list.files(paste(path,Countries[i],"STATS\\",sep="\\"))
		ctry.items <- sapply(ctry.files,txtparse)
		items.in.Countries <- c(items.in.Countries,possible.items[which(possible.items%in%ctry.items)])
	}
	
	# those items common to all countries will have table values equal to length(Countries)
	CT <- table(items.in.Countries)
	items.in.all.Countries <- sort(names(CT[which(CT==length(Countries))]))
	...
	for (i in 1:length(Countries)){
		...
		for (j in 1:length(wanteditems)){
			pathj <- paste(path,"\\",Countries[i],"\\STATS\\",wanteditems[j],".txt",sep="")
			itemj <- read.table(file=pathj,fill=T,skip=1,as.is=T,header=T,na.string=".")
	...
	# syntax return:
	if (syntax == T){
		options(useFancyQuotes=F) # so that quotes properly appear in cat()
		...
		for (i in 1:length(wanteditems)){
			...
			if (any(sapply(years[[i]],grepl,pattern="-"))){
				...
				if (length(years[[i]])==(as.numeric(years[[i]][length(years[[i]])])-as.numeric(years[[i]][1])+1)){
		...
		cat(paste("DATA <- HMDget(countries = c(",countriesstring,"), wanteditems = c(",wanteitemsstring,"), years = years, column =",dQuote(column),", drop.tadj = ",drop.tadj,", format = ",format,", path =",dQuote(path),")\n\n(just copy and paste it next time)\n",sep=""))
		
	}
	
	gc() # clean up memory wasted
	return(output)
}
...
HMD2Rweb <- function(countries=NULL, wanteditems=NULL, years=TRUE, column=NULL, drop.tadj=TRUE, format=0 , username, password, syntax=TRUE){
	...
	greplavail <- function(possible.items,STATShtml){
		possible.items[which(sapply(possible.items,grepl,x=STATShtml)==T)]
	}
	...
	items.in.Countries <- c()
	for (i in 1:length(Countries)){
		this.url <- paste("http://www.mortality.org/hmd/",Countries[i],"STATS/",sep="/")
	...
	f.fetchit2 <- function(stat=this.stat,this.skip=2,countries,handle){
		...
		this.url<- paste(urlbase,countries,"STATS",this.stat.txt,sep="/")
		stat.return<- NULL;
		x<-NULL;
	...
	if (syntax == TRUE){
		...
		cat(paste("DATA <- HMDget(countries = c(",countriesstring,"), wanteditems = c(",wanteitemsstring,"), years = years, column =",dQuote(column),", drop.tadj = ",drop.tadj,", format = ",format,", username = ",dQuote(username),", password = ",dQuote(password),")\n\n(just copy and paste it next time)\n",sep=""))
	}
	gc() # clean up memory wasted
	return(output)
}
...
if ((missing(path) & (missing(password) | missing(username))) | (missing(path)==FALSE & missing(password)==FALSE & missing(username)==FALSE)){
	...
	if (missing(path)==FALSE & (missing(password) | missing(username))){
		local <- TRUE
	} else {
		# if path is missing and both username and password are given, then it's web
		if (missing(path) & (missing(password) | missing(username))==FALSE){
			local <- FALSE
		}
	}
} 
...
if (local==TRUE){
	output <- HMD2Rlocal(countries=countries,wanteditems=wanteditems, years=years, column=column, drop.tadj=drop.tadj, format=format,path=path,syntax=syntax)
} else {
	...
	output <- HMD2Rweb(countries=countries,wanteditems=wanteditems, years=years, column=column, drop.tadj=drop.tadj, format=format, username=username, password=password, syntax=syntax)
}
return(output)
}
